  ...
  try {
    ...
    if (state === "process") {
      sendMail(
        ...
        PROCESS_ORDER(name, last_name, id)
      );
    }
    if (state === "cancel") {
      sendMail([email], "Purchase canceled.", CANCEL_ORDER(name, last_name, id));
    }
    if (state === "finish") {
      sendMail([email], "Purchase completed.", FINISH_ORDER(name, last_name, id));
    }
...
export const checkout = async (
  ...
  try {
    ...
    if (!control.includes(true)) {
      ...
      const addressText = `Province: ${address?.toJSON().province} ,City: ${address?.toJSON().city
        } , Locality: ${address?.toJSON().locality} ,Street number:  ${address?.toJSON().street_number
      ...
      sendMail(
        ...
        SUCCESS_ORDER(
          customer.name,
          customer.last_name, 
          `${req.headers.origin}/orders/${order.toJSON().id}`
        )
      );
